...
SD_Error SD_PowerON(void)
{
  ...
  SDIO_InitStructure.SDIO_ClockDiv            = SDIO_INIT_CLK_DIV;                // SDIO分频系数，在识别阶段需要配置为400KHz
  SDIO_InitStructure.SDIO_ClockEdge           = SDIO_ClockEdge_Rising;            // 上升沿有效
  SDIO_InitStructure.SDIO_ClockBypass         = SDIO_ClockBypass_Disable;         // 是否使用Bypass，Bypass意思是HCLK不分频直接连接SDIO_CK
  SDIO_InitStructure.SDIO_ClockPowerSave      = SDIO_ClockPowerSave_Disable;      // 省电模式，意思是空闲时间不输出SDIO_CK时钟
  SDIO_InitStructure.SDIO_BusWide             = SDIO_BusWide_1b;                  // 数据线数量，暂时配置为1根
  SDIO_InitStructure.SDIO_HardwareFlowControl = SDIO_HardwareFlowControl_Disable; // 关闭硬件流控制（硬件流控制意思是当FIFO数据溢出时，向从机发送暂停信号）
  SDIO_Init(&SDIO_InitStructure);
  ...
  /* 发送Cmd0命令：复位所有SD卡，所有SD卡进入空闲状态 */
  /*!< CMD0: GO_IDLE_STATE ---------------------------------------------------*/
  /*!< No CMD response required */
  SDIO_CmdInitStructure.SDIO_Argument = 0x0;                    // 命令参数
  SDIO_CmdInitStructure.SDIO_CmdIndex = SD_CMD_GO_IDLE_STATE;   // 命令号
  SDIO_CmdInitStructure.SDIO_Response = SDIO_Response_No;       // 响应类型
  SDIO_CmdInitStructure.SDIO_Wait     = SDIO_Wait_No;           // 设置等待模式，即 SDIO_CMD 寄存器的 WAITPEND 位和 WAITINT 位等待什
                                                                // 么事件（一共三种事件：超时、中断、传输完成）
  SDIO_CmdInitStructure.SDIO_CPSM     = SDIO_CPSM_Enable;       // CPSM在发送命令前等待上一轮数据传输结束
  ...
  if (errorstatus == SD_OK) // 如果接收到了响应则一切OK
  {
    CardType = SDIO_STD_CAPACITY_SD_CARD_V2_0; /*!< SD Card 2.0 */
    SDType = SD_HIGH_CAPACITY;
  }
  else                      // 如果没接收到响应，则表示SD卡非2.0或mmc卡，或者电压不匹配
  {
    /*!< CMD55 */
  ...
  if (errorstatus == SD_OK)
  {
    /* 反复发送ACMD41命令，直到读出validvoltage或者超过次数上限 */
    /*!< SD CARD */
    ...
    while ((!validvoltage) && (count < SD_MAX_VOLT_TRIAL))
    {
      ...
      /* R3响应，响应中包含SD卡的OCR寄存器值（OCR包含电压范围、SD卡容量型号（大容量/标准容量）） */
...
/**
  * @brief  Allows to process all the interrupts that are high.数据传输结束中断
  * @param  None
  * @retval SD_Error: SD Card Error code.
  */
SD_Error SD_ProcessIRQSrc(void)
{
  /* StopCondition为1代表多块读写 */
  if (StopCondition == 1)
  {
    SDIO->ARG = 0x0;
    SDIO->CMD = 0x44C;  // CMD12: 结束读写
